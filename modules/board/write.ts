...
export const writeRequest = createAction(WRITE_REQUEST, (data: any) => data)
export const writeCancelled = createAction(WRITE_CANCELLED, (data: any) => data)
...
function* writeArticle(action: { payload: any; }){
    try {
...
const writeAPI = (payload: any) => axios.post(
    `${SERVER}/board/write`,
    payload,
    {headers}
)

function* writeCancel(action: any){
...
const write = handleActions({
    ...
    [WRITE_SUCCESS]: (state: any, action: { payload: any; }) => ({
        ...state,
        article: action.payload,
        isWritten: true
    }),
    [WRITE_FAILURE]: (state: any, action: { payload: any; }) => ({
        ...state,
